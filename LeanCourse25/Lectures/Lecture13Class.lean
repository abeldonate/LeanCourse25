...
example (g g' : G) (x : X) : (g * g') • x = g • (g' • x) := by exact mul_smul g g' x -- sorry
example (x : X) : (1 : G) • x = x := by exact MulAction.one_smul x -- sorry
...
example {g g' : G} : g • g' = g * g' := by rw [smul_eq_mul]
...
example (v : V) : (2 : K) • v = (2 : ℕ) • v := by exact ofNat_smul_eq_nsmul K 2 v
  -- Nat.cast_smul_eq_nsmul K 2 v
example (v : V) : (2 : ℤ) • v = (2 : K) • v := by simp [← Int.cast_smul_eq_zsmul K 2 v]
...
-- to the range of f.
example (f : M →ₗ[R] N) : (M ⧸ (LinearMap.ker f)) ≃ₗ[R] LinearMap.range f := by
  exact f.quotKerEquivRange

-- In particular, for a surjective linear map, the quotient of M by ker f is equivalent to N.
example {f : M →ₗ[R] N} (hf : Surjective f) : (M ⧸ (LinearMap.ker f)) ≃ₗ[R] N := by
...
example (hv : End.HasEigenvector f a v) : End.HasEigenvalue f a :=
  by exact End.hasEigenvalue_of_hasEigenvector hv
